...
gulp.task 'sign:osx64', ['build:osx64'], ->
  shelljs.exec 'codesign -v -f -s "jpagnano apps" ./build/FatCatQueue/osx64/FatCatQueue.app/Contents/Frameworks/*'
  shelljs.exec 'codesign -v -f -s "jpagnano apps" ./build/FatCatQueue/osx64/FatCatQueue.app'
  shelljs.exec 'codesign -v --display ./build/FatCatQueue/osx64/FatCatQueue.app'
  shelljs.exec 'codesign -v --verify ./build/FatCatQueue/osx64/FatCatQueue.app'
...
gulp.task 'pack:osx64', ['sign:osx64'], ->
  ...
  shelljs.rm '-f', './dist/FatCatQueue.dmg'   # appdmg fails if the dmg already exists

  gulp.src []
    .pipe require('gulp-appdmg')
      source: './assets-osx/dmg.json'
      target: './dist/FatCatQueue.dmg'
...
[32, 64].forEach (arch) ->
  ['deb', 'rpm'].forEach (target) ->
    gulp.task "pack:linux#{arch}:#{target}", ['build:linux' + arch], ->
      ...
      move_opt = gulp.src [
        './assets-linux/fcqueue.desktop'
        './assets-linux/after-install.sh'
        './assets-linux/after-remove.sh'
        './build/FatCatQueue/linux' + arch + '/**'
      ]
        .pipe gulp.dest './build/linux/opt/fcqueue'

      move_png48 = gulp.src './assets-linux/icons/48/fcqueue.png'
        .pipe gulp.dest './build/linux/usr/share/icons/hicolor/48x48/apps'

      move_png256 = gulp.src './assets-linux/icons/256/fcqueue.png'
        .pipe gulp.dest './build/linux/usr/share/icons/hicolor/256x256/apps'

      move_svg = gulp.src './assets-linux/icons/scalable/fcqueue.svg'
      ...
      mergeStream move_opt, move_png48, move_png256, move_svg
        .on 'end', ->
          ...
          output = "../../dist/FatCatQueue_linux#{arch}.#{target}"

          shelljs.mkdir '-p', '../../dist' # it fails if the dir doesn't exist
          shelljs.rm '-f', output          # it fails if the package already exists

          shelljs.exec "fpm -s dir -t #{target} -a #{port} --rpm-os linux -n fcqueue --after-install ./opt/fcqueue/after-install.sh --after-remove ./opt/fcqueue/after-remove.sh --license MIT --category Chat --url \"https://example.com\" --description \"A sample NW.js app.\" -m \"Alexandru Rosianu <me@aluxian.com>\" -p #{output} -v #{manifest.version} ."
          shelljs.cd '../..'
...
gulp.task 'run:osx64', ['build:osx64'], ->
  shelljs.exec 'open ./build/FatCatQueue/osx64/FatCatQueue.app'
